        ...
        if (stagesTrackArray !== null) {
          ...
          stagesTrackArray.forEach(async (stage, k) => {
            ...
            if (trkptStr.length > 0) {
              ...
              // Record trkpt elements
              let recordsTrkptArr = ["time", "magvar", "geoidheight", "name", "cmt", "desc", "src", "link", "url", "urlname", "sym", "type", "fix", "sat", "hdop", "vdop", "pdop", "ageofdgpsdata", "dgpsid", "extensions"];

              recordsTrkptArr.map(async (element) => {

                // Record trkpt elements
                let elementArr = await exports.getTagsValueArr(trkptStr, `<${element}>`, `</${element}>`);
              ...
              // Records "name", "type", "cmt", "desc", "src", "url", "urlname", "number" tags
          ...
          // Console message
          console.log(":( Gpx file is wrong. Check your gpx file.".red);
        }
      } catch(error) {
          console.log(':( getStagesTrack error'.red);
          reject(console.log);
...
exports.trackDistanceCalculation = async (positionsArray) => {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      let d = positionsArray.map(async (position, l) => {
      ...
      let totalDistance = d.reduce((a, b) => a + b, 0);
